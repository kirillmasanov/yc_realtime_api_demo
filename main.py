...
BASE_SESSION: dict = {
    "instructions": (
        ...
        "и поиск по загруженным документам (file_search, если пользователь загрузил файл). "
        "Используй web_search для новостей, актуальных событий и любой информации, которой у тебя может не быть. "
        "Используй file_search для любых вопросов о содержимом загруженного документа — "
        "всегда обращайся к документу, не отвечай по памяти если он загружен. "
        "Если инструмент file_search недоступен — сообщи пользователю, что файл не загружен. "
