...
- Настройки голоса, чувствительности VAD и системного промпта прямо из браузера
...
| `POST` | `/api/upload` | Загрузить файл для RAG (multipart: `file`, `session_id`) |
| `GET` | `/api/rag/status` | Текущее состояние RAG для сессии |
| `DELETE` | `/api/rag/clear` | Удалить файл и векторный индекс |
...
При получении события `session.created` сервер сразу отправляет `session.update` с конфигурацией сессии: инструкции, голос, параметры VAD, список инструментов.
...
3. Сервер добавляет инструмент `file_search` в конфиг сессии и отправляет обновлённый `session.update` в Yandex.
