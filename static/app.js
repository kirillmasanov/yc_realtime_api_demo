...
function handleMessage(event) {
  ...
  switch (type) {
    ...
    // Tool call (custom event from server) — store icon for next assistant bubble
    case "tool_call":
      pendingToolIcon = msg.name;
      break;
...
const TOOL_ICONS = {
  web_search: '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
  calculator: '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/><circle cx="9" cy="12" r="1.2" fill="currentColor" stroke="none"/><circle cx="12" cy="12" r="1.2" fill="currentColor" stroke="none"/><circle cx="15" cy="12" r="1.2" fill="currentColor" stroke="none"/><circle cx="9" cy="16" r="1.2" fill="currentColor" stroke="none"/><circle cx="12" cy="16" r="1.2" fill="currentColor" stroke="none"/><circle cx="15" cy="16" r="1.2" fill="currentColor" stroke="none"/></svg>',
  file_search: '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>',
};
...
function appendAssistantText(delta) {
  if (!currentAssistantBubble) {
    ...
    if (pendingToolIcon) {
      ...
      badge.innerHTML = TOOL_ICONS[pendingToolIcon] || "";
      badge.title = pendingToolIcon.replace("_", " ");
      currentAssistantBubble.appendChild(badge);
      pendingToolIcon = null;
    }

    currentAssistantTextSpan = document.createElement("span");
    currentAssistantBubble.appendChild(currentAssistantTextSpan);
    chat.appendChild(currentAssistantBubble);
  }
  currentAssistantText += delta;
  currentAssistantTextSpan.textContent = currentAssistantText;
  scrollToBottom();
}
